...
public class ImageUtils {
    ...
    public static List<String> pdf2Image(String filePath) throws Exception {
        ...
        if(filePath.contains("/")){
            pre = filePath.substring(0,filePath.lastIndexOf("/") + 1);
        }else {
            pre = filePath.substring(0,filePath.lastIndexOf("\\") + 1);
        }
        ...
        for (int i = 0; i < document.getNumberOfPages(); i++) {
            BufferedImage image = (BufferedImage) document.getPageImage(i, GraphicsRenderingHints.SCREEN,
                    Page.BOUNDARY_CROPBOX, 0F, 1.5F);
            File imageFile = new File(pre + fileName + (i + 1) + PNG_SUFFIX);
